...
#
# Optional overrides:
#   MAX_EPOCHS=15 sbatch train_hope3.sh
#   LATENT_ACTION_DIM=8 sbatch train_hope3.sh
#   TRAIN_RUN_NAME=hi_lewm_p2_train_hope3_custom sbatch train_hope3.sh
#   SCRATCH_STABLEWM_HOME=/scratch-shared/$USER/stablewm_data sbatch train_hope3.sh

#SBATCH --partition=gpu_a100
#SBATCH --constraint=scratch-node
#SBATCH --gpus=1
#SBATCH --job-name=hi_l2_pusht_train_latent_action_dim_8
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=9
#SBATCH --time=10:00:00
#SBATCH --output=output/hope2/train_latent_action_dim_8_%j.out
#SBATCH --error=output/hope2/train_latent_action_dim_8_%j.err
...
if [[ -z "${WANDB_API_KEY:-}" ]]; then
  ...
  echo "Set it in ${WANDB_ENV_FILE} or submit with: sbatch --export=ALL,WANDB_API_KEY=<your_key> train_hope3.sh" >&2
  exit 2
fi
...
TRAIN_RUN_NAME="${TRAIN_RUN_NAME:-hi_lewm_p2_train_hope3_${SLURM_JOB_ID:-manual}}"
