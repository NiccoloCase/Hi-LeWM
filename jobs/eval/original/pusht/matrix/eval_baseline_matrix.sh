...
#
# Output layout:
# - Logs: `jobs/eval/original/pusht/matrix/logs/<sweep-name>/`
# - Eval artifacts: `${STABLEWM_HOME}/pusht/<sweep-name>/<task-label>/`
...
#SBATCH --job-name=orig_pusht_matrix
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=08:00:00
#SBATCH --chdir=/gpfs/home2/scur0200/main/jobs/eval/original/pusht/matrix
...
resolve_matrix_dir() {
  ...
  for candidate in     "${MATRIX_DIR:-}"     "${SLURM_SUBMIT_DIR:-}"     "${PWD:-}"     "${PROJECT_ROOT:-}/jobs/eval/original/pusht/matrix"     "/gpfs/home2/${USER}/main/jobs/eval/original/pusht/matrix"; do
...
if ! MATRIX_DIR_RESOLVED="$(resolve_matrix_dir)"; then
  echo "ERROR: could not resolve jobs/eval/original/pusht/matrix." >&2
  exit 2
fi
...
DATASET_PATH="${DATASET_PATH:-${STABLEWM_HOME}/pusht_expert_train.h5}"
...
RESULT_FILENAME="pusht_results_${DISPLAY_LABEL}.txt"
...
echo "Eval dataset: pusht_expert_train"
