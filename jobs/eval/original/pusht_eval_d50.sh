...
#SBATCH --output=out/pusht_50_eval_%j.out
#SBATCH --error=out/pusht_50_eval_%j.err
...
cd third_party/lewm
python eval.py --config-name="${CONFIG_NAME}" policy="${POLICY}" eval.goal_offset_steps=50 eval.eval_budget=100

echo "Original eval finished."
echo "Result file should be under: ${STABLEWM_HOME}/$(dirname "${POLICY}")/d50/pusht_results.txt"
